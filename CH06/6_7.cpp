...
Token Token_stream::get()
{
	...
	case '#': //new declaration keyword
		...
		if (isalpha(ch)) //is ch a letter
			...
			if (s == "const")
				return (Token(ct));
			if (s == "help") // I put this here to avoid words beginning with h to be mistaken as help commands
				return (Token(help));
...
void print_help()
{
	std::cout <<"-INSTRUCTIONS-\nEnter an arithmetical expression (or several followed by ; each) to compute its results.\n"
		  ...
		  <<"Enter \"quit\" to exit and \"help\" to see the instructions again.\n";
}
...
try 
{
	...
	std::cout << "-CALCULATOR-\nPress \"help\" for help and \"quit\" to exit the program.\n";
	calculate();
	return 0;
}
